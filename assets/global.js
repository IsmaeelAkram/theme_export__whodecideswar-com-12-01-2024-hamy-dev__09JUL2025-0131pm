...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    console.log(this.currentVariant, this.currentVariant.available);
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    const price = productForm.querySelector("#price-hidden").innerHTML;
    const priceHover = productForm.querySelector("#price-hover");
    if (!addButton) return;

    if (disable) {
      addButton.setAttribute("disabled", "disabled");
      addButton.classList.add("sold-out-button");
      addButton.classList.remove("add-button");
      if (text) addButtonText.textContent = price + " - " + text;
    } else {
      addButton.removeAttribute("disabled");
      addButton.classList.remove("sold-out-button");
      addButton.classList.add("add-button");
      priceHover.textContent = window.variantStrings.addToCart;
      addButtonText.textContent = price;
    }
...
// --- GALLERY BUY BUTTON VARIANT FIX ---
document.addEventListener("variantChange", function (e) {
  if (!e.detail || !e.detail.html || !e.detail.sectionId) return;
  var sectionId = e.detail.sectionId;
  var newHtml = e.detail.html;
  var newPrice = newHtml.getElementById("price-" + sectionId);
  if (!newPrice) return;
  document.querySelectorAll("#price-" + sectionId).forEach(function (el) {
    el.innerHTML = newPrice.innerHTML;
  });
});
